...
const Page = ({}) => {
    ...
    const requestOtp = async() => {
        try {
            ...
            console.log("running...........")
            await axios.post("/api/users/forgot-password-otp",user);
            console.log("ok...........")
            setShowAll(true);
            setLoading(false);
        } catch (error:any) {
            console.log("Request for otp failed");
            toast.error(error.response.data.error);
        }
    }

    useEffect(()=>{
        if(user.email.length>0 && user.password.length>0 && user.otp.length>0){
            setButtonDisabled(false);
        }
        else{
            setButtonDisabled(true);
        }
        if(user.email.length>0 && !showAll){
            setSecondButtonDisabled(false);
        }
        else if(user.email.length==0 && !showAll){
            setSecondButtonDisabled(true);
        }
    },[user]);
    ...
    return( 
        <div className="min-h-screen bg-black py-12 pt-32 text-white flex flex-col justify-center items-center">
            ...
            <div className="max-w-64 sm:max-w-md w-full mx-auto rounded-2xl p-4 md:p-8 shadow-input 
                ...
                <form className="my-8" onSubmit={(e)=>onChange(e)}>
                    ...
                    {showAll && <LabelInputContainer className="mb-4">
